...
st.title("ヴェロビ 復習（全体累積）｜2車複 1-234/2-134/3-124 軸連対率差 v5.7｜7車固定・欠車対応")
...
with tabs[0]:
    ...
    with st.form("daily_input_form"):
        cols_hdr = st.columns([0.55, 0.75, 0.9, 2.75, 1.05, 1.15])
        ...
        for i in range(1, 37):
            c1, cvenue, c2, c3, c4, c5 = st.columns([0.55, 0.75, 0.9, 2.75, 1.05, 1.15])
    ...
    for item in daily_inputs:
        ...
        any_input = any([venue_memo.strip(), vline.strip(), fin.strip(), pay_2f > 0])
        if any_input:
            if not vorder:
